...
const RPMayores = () => {
    ...
    return (
        <ContractLayout
            title="Recursos Propios Mayores"
            description="Cargue el Excel y genere el contrato de recursos propios para mayores."
            users={users}
        ...
        >
            ...
            <div className="grid grid-cols-2 gap-2 mt-2">
                <button className="primary-button flex items-center justify-center gap-2 p-2.5 rounded-md bg-blue-600 text-white hover:bg-blue-700 transition-colors disabled:opacity-50 text-xs font-bold"
                    onClick={async () => {
                        const data = validateAndPrepare();
                        if (!data) return;
                        const nameToUse = customFileName ? (customFileName.toLowerCase().endsWith('.docx') ? customFileName : `${customFileName}.docx`) : `Contrato_RP_Mayores_${selectedUserData.name.replace(/\s+/g, '_')}.docx`;
                ...
                <button className="primary-button flex items-center justify-center gap-2 p-2.5 rounded-md bg-red-600 text-white hover:bg-red-700 transition-colors disabled:opacity-50 text-xs font-bold"
                    onClick={() => {
                        const data = validateAndPrepare();
                        if (!data) return;
                        const nameToUse = customFileName ? (customFileName.toLowerCase().endsWith('.pdf') ? customFileName : `${customFileName}.pdf`) : `Contrato_RP_Mayores_${selectedUserData.name.replace(/\s+/g, '_')}.pdf`;
            ...
            <button className="secondary-button mt-2 flex items-center justify-center gap-2 w-full p-2.5 rounded-md border border-primary text-primary hover:bg-primary/10 text-sm font-bold"
                onClick={async () => {
                    const data = validateAndPrepare();
                    if (!data) return;
                    const nameToUse = customFileName ? (customFileName.toLowerCase().endsWith('.docx') ? customFileName : `${customFileName}.docx`) : `Contrato_RP_Mayor_${selectedUserData.name}.docx`;
                    toast.promise(async () => {
