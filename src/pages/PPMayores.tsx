...
const PPMayores = () => {
    ...
    return (
        <ContractLayout
            title="Pronto Pago Mayores"
            description="Cargue el Excel y genere el contrato de pronto pago para mayores."
            users={users}
            ...
            <div className="grid grid-cols-2 gap-2 mt-2">
                <button className="primary-button flex items-center justify-center gap-2 p-2.5 rounded-md bg-blue-600 text-white hover:bg-blue-700 transition-colors disabled:opacity-50 text-xs font-bold"
                    onClick={async () => {
                        const data = validateAndPrepare();
                        if (!data) return;
                        const nameToUse = customFileName ? (customFileName.toLowerCase().endsWith('.docx') ? customFileName : `${customFileName}.docx`) : `Contrato_PP_Mayores_${selectedUserData.name.replace(/\s+/g, '_')}.docx`;
                        await generateContract("/contratos/Condiciones Específicas-Pronto Pago Mayor de Edad.docx", data, nameToUse);
                ...
                <button className="primary-button flex items-center justify-center gap-2 p-2.5 rounded-md bg-red-600 text-white hover:bg-red-700 transition-colors disabled:opacity-50 text-xs font-bold"
                    onClick={() => {
                        const data = validateAndPrepare();
                        if (!data) return;
                        const nameToUse = customFileName ? (customFileName.toLowerCase().endsWith('.pdf') ? customFileName : `${customFileName}.pdf`) : `Contrato_PP_Mayores_${selectedUserData.name.replace(/\s+/g, '_')}.pdf`;
