...
const LumniMenores = () => {
  ...
  return (
    <ContractLayout
      title="Lumni Menores de edad"
      description="Cargue el Excel y genere el contrato de financiación Lumni para menores."
      users={users}
      selectedUser={selectedUser}
      onUserSelect={setSelectedUser}
      onFileUpload={handleFileUpload}
      previewBlob={previewBlob}
      templateUrl="/contratos/Condiciones Específicas-Financiación Lumni- Menor de Edad.docx"
    >
      ...
      <div className="grid grid-cols-2 gap-2 mt-2">
        <button className="primary-button flex items-center justify-center gap-2 p-2.5 rounded-md bg-blue-600 text-white hover:bg-blue-700 transition-colors disabled:opacity-50 text-xs font-bold"
          onClick={async () => {
            const data = validateAndPrepare();
            if (!data) return;
            const nameToUse = customFileName ? (customFileName.toLowerCase().endsWith('.docx') ? customFileName : `${customFileName}.docx`) : `Contrato_Lumni_Menores_${selectedUserData.name.replace(/\s+/g, '_')}.docx`;
            await generateContract("/contratos/Condiciones Específicas-Financiación Lumni- Menor de Edad.docx", data, nameToUse);
        ...
        <button className="primary-button flex items-center justify-center gap-2 p-2.5 rounded-md bg-red-600 text-white hover:bg-red-700 transition-colors disabled:opacity-50 text-xs font-bold"
          onClick={() => {
            const data = validateAndPrepare();
            if (!data) return;
            const nameToUse = customFileName ? (customFileName.toLowerCase().endsWith('.pdf') ? customFileName : `${customFileName}.pdf`) : `Contrato_Lumni_Menores_${selectedUserData.name.replace(/\s+/g, '_')}.pdf`;
      ...
      <button className="secondary-button mt-2 flex items-center justify-center gap-2 w-full p-2.5 rounded-md border border-primary text-primary hover:bg-primary/10 text-sm font-bold"
        onClick={async () => {
          const data = validateAndPrepare();
          if (!data) return;
          const nameToUse = customFileName ? (customFileName.toLowerCase().endsWith('.docx') ? customFileName : `${customFileName}.docx`) : `Contrato_Lumni_Menor_${selectedUserData.name}.docx`;
          toast.promise(async () => {
            const blob = await generateContract("/contratos/Condiciones Específicas-Financiación Lumni- Menor de Edad.docx", data, "contrato.docx", true);
